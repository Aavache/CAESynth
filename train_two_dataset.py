""" 
TRAINING Script Timbre Pitch Disentanglement by iterating over two datasets
"""
# External libs
import argparse
import numpy as np
...
from data.fsd_nsynth_loader_tmp import NsynthFSD
...
def main():
    ...
    # Training set
    fsd_nsynth_trainset = NsynthFSD(opt['data'],  size=training_size)  # create a dataset according to the options file
    ...
    # Validation set
    fsd_nsynth_valset = NsynthFSD(opt['data'], size=validation_size, is_train=False)  # create a dataset according to the options file
    ...
    for epoch in range(train_opt['start_epoch'], train_opt['n_epochs'] + 1):    # outer loop for different epochs
        ...
        for data in train_prg:  # inner loop within one epoch
            ...
            if train_iter % 32 == 0:# TODO: when using batch 1 is too fast
